import { ProcessedWeatherData, ComfortScoreBreakdown } from '../types/weather';
...
class ComfortIndexService {
  ...
  private calculateTemperatureScore(temp: number): number {
    ...
    // Use steeper penalty: 10 points per degree 
  ...
  private calculateHumidityScore(humidity: number): number {
    // Linear penalty: 2.5 points per percentage point deviation 
  ...
  private calculateWindSpeedScore(windSpeed: number): number {
    // Linear penalty: 20 points per m/s deviation 
